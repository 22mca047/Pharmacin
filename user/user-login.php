...
if($_POST)
{
    ...
    $selectq = mysqli_query($connection,"select * from tbl_customer where customer_email='{$email}' and customer_password ='{$password}'") or die(mysqli_error($connection));
    $count = mysqli_num_rows($selectq);
    $row = mysqli_fetch_array($selectq);
    if($count>0)
    {
        $_SESSION['ci'] = $row['customer_id'];
        $_SESSION['cn'] = $row['customer_name'];
        $_SESSION['cg'] = $row['customer_gender'];
        $_SESSION['cd'] = $row['customer_dob'];
        $_SESSION['cimage'] = $row['customer_image'];
        $_SESSION['ca'] = $row['customer_address'];
        $_SESSION['ce'] = $row['customer_email'];
        $_SESSION['careaid'] = $row['area_id'];
        echo "<script>alert('You Have Successfully Logged In..');window.location='home.php';</script>";
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12" >
                                ...
                                <div class="p-a30 m-b30 border-2 contact-area align-self-stretch">
                                    <h3 class="form-title m-t0" style="text-align:center;">User Login</h3>
                                    ...
                                    <div class="form-group" style="margin-left:50px;">
                                        ...
                                        <button type="button" class="site-button dz-xs-flex col-lg-5 text-center" onclick="window.location='user-register.php';">Register </button>
                                    </div>
                                    
                                    
                                    <div class="form-group text-left pass-check">
                                        <a  href="user-forgotpassword.php" class="m-l15"><i class="fa fa-unlock-alt"></i> Forgot Password</a> 
                                    </div>
                                </div>
                                </form>
                            </div>

...
if ($_POST) {
    ...
    $oldpassq = mysqli_query($connection, "select vendor_password from tbl_vendor where vendor_id='{$_SESSION['vi']}'") or die(mysqli_error($connection));
    $oldpassdb = mysqli_fetch_array($oldpassq);
    //old password condition
    if ($oldpassdb['vendor_password'] == $opass) {
        ...
        if ($npass == $cnpass) {
            ...
            if ($opass == $npass) {
                ...
                $updateq = mysqli_query($connection, "update tbl_vendor set vendor_password='{$npass}' where vendor_id='{$_SESSION['vi']}'") or die(mysqli_error($connection));
                if ($updateq) {
                    echo "<script>alert('Password Changed');window.location='log-out.php';</script>";
                }
            }
        } else {

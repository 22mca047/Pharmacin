...
if ($_POST) {
    ...
    $query = mysqli_query($connection, " update tbl_area set area_name ='{$aname}',area_pincode ='{$apincode}' where area_id='{$id}'") or die(mysqli_error($connection));

    if ($query) {
        echo "<script>alert('Record Updated');window.location='manage-area.php';</script>";
    }
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <head>
        ...
        <title>PHARMACIN - Edit Area </title>
        <!-- Favicon icon -->
                        ...
                        <div class="col-xl-12 col-xxl-12 col-sm-12">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title">Edit Area Info</h5>
                                </div>
                                <div class="card-body">
                                    <form action="#" method="post" enctype="multipart/form-data" id="myform">
                                        <div class="col-xs-12">

                                            <input type="hidden" name="id" value="<?php echo $selectrow['area_id']; ?>">
                                            <div class="form-group">
                                                <label class="form-label" for="add-name">Area Name</label>
                                                <input type="text" class="form-control" id="area-name" value="<?php echo $selectrow['area_name']; ?>" name="aname" placeholder="Enter Area Name" required>
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label" for="add-name">Area Pincode</label>
                                                <input type="number" class="form-control" id="area-pincode" value="<?php echo $selectrow['area_pincode']; ?>" name="apincode" placeholder="Enter Area Pincode" required>
                                            </div>
                                        </div>
                                        <div class="col-xs-12">
                                            <button type="submit" class="btn btn-primary" style="margin-left: -1px;margin-right: -11px;">Update Area</button>
